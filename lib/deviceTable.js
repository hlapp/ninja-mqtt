...
module.exports.findDevice = function(query) {
    ...
    if (query.D) matches = matches.filter(function(devEntry) {
        return (devEntry.D === query.D);
    });
    if (query.V) matches = matches.filter(function(devEntry) {
        return (devEntry.V === query.V);
    });
    if (query.devType) matches = matches.filter(function(devEntry) {
        return (devEntry.devType === query.devType);
    });
    if (query.name) matches = matches.filter(function(devEntry) {
